...
namespace GearHonService.ViewModels
{
	public partial class ServiceReportViewModel : BaseViewModel
	{
		...
		public async Task CreateExcelFile()
		{
			...
			//ask user if he want to save the service report to database
			var result2 = await Shell.Current.DisplayAlert("Save Service Report", "Do you want to save the Service Report to the database?", "Yes", "No");
			if (result2)
			{
				//save service report to database
				await SaveServiceReport();
			}
			else
			{
				//warning that the service report is not saved
				await Shell.Current.DisplayAlert("Warning", "The Service Report is not saved to the database! There for it will be not used in your invoice", "Ok");
			}
		}

		private async Task SaveServiceReport()
		{
			try
			{
				if (ID == 0)
				{
					var insertServiceReport = new ServiceReportModel
					{
						UID = UID,
						//TODO: Implement a get contrctors and let the user choose
						Contractor = "Präwema",
						CustomerName = CustomerName,
						Month = Convert.ToString(ServiceStartDate.Month),
						Year = Convert.ToString(ServiceStartDate.Year),
						TotalHour = TotalAllTime
					};

					try
					{
						var insertResult = await _supabaseClient
						.From<ServiceReportModel>()
						.Insert(insertServiceReport);

						await Shell.Current.DisplayAlert("Success", "Service reprot successfully added", "Ok");
					}
					catch (Exception ex)
					{
						await Shell.Current.DisplayAlert("Error", ex.Message, "Ok");
